...
module.exports.edit = {
    handler: async (request, reply) => {
        try {
            await Vehicles.update({ _id: request.params.id }, request.payload);
        } catch (err) {
            return reply({ result: 'it was not possible to update your document', err })
                .code(400);
        }

        return reply({ result: { message: 'success on updating', data: await Vehicles.findById(request.params.id) } }).code(201);
    },
    validate: {
        payload: vehiclesPostPayloadValidation
    }
